...
public class ExcelXlsReader implements HSSFListener {
	...
	public ExcelXlsReader(String resultFilePath, String resultFileName) {
	...
	/**
	 * 遍历excel下所有的sheet
	 *  String fileName 读取绝对路径下的文件时使用
	 * @param fileName
	 * @throws Exception
	 */
	public int process(MultipartFile file/*String fileName*/) throws Exception {
		//filePath = fileName;
		...
		if (record instanceof LastCellOfRowDummyRecord) {
			...
			if (flag) { //该行不为空行且该行不是第一行，发送（第一行为列名，不需要）
				if (curRow == 0 ){
					...
					ExcelReaderUtil.sendRows(filePath, sheetName, sheetIndex, curRow + 1, cellList,resultFilePath,resultFileName); //每行结束时，调用sendRows()方法
					totalRows++;
				}
			}
			//清空容器
